pipeline {
  ...
  environment {
    ...
    JSON_URL = 'https://api.github.com/repos/caronc/apprise-api/tags'
    JSON_PATH = '.[0].name'
    BUILD_VERSION_ARG = 'APPRISE_TAG'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-apprise-api'
    CONTAINER_NAME = 'apprise-api'
    DOCKERHUB_IMAGE = 'linuxserver/apprise-api'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/apprise-api'
    PR_DOCKERHUB_IMAGE = 'lspipepr/apprise-api'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='true'
    CI_WEB='true'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/apprise-api") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/apprise-api",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-apprise-api",\
                       "backendUrl":"https://ghcr.io/linuxserver/apprise-api",\
                       "publicUrl":"https://lscr.io/linuxserver/apprise-api"}' || :
              else
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker buildx build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-apprise-api/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-apprise-api\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-apprise-api\" \
